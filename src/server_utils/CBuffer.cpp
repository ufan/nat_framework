...
CBuffer::CBuffer(uint32_t buf_size)
    : p_(NULL), size_(buf_size), head_(0), tail_(0) {
...
/**
 * @brief Write to the buffer if remaining space is big enough.
 * If not, nothing will be done and return false.
 * Should check the return status to confirm whether data has been successfully
...
/**
 * @brief Return the start address of unread data in the buffer, together with
 * the length of the block which is suitable for continuous reading.
 */
