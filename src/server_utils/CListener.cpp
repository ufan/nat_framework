...
{
	if(-1 == (fd_ = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0)))
	{
    ...
    // set sd reuseful
    int reuseaddr = 1;
    if (setsockopt(fd_, SOL_SOCKET, SO_REUSEADDR, (const char*) &reuseaddr, sizeof(reuseaddr)) != 0)
    {
    	::close(fd_);
    	fd_ = -1;
...
void CListener::read_cb(EV_P_ ev_io* w, int events)
{
	...
	int fd = accept4(w->fd, (struct sockaddr *)&addr, &addrlen, SOCK_NONBLOCK | SOCK_CLOEXEC);
	if(fd < 0)
	{
