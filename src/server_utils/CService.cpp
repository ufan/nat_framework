...
CService::CService(uint32_t rbuf_size, uint32_t wbuf_size) : head_size_(0), pkg_size_(0), state_(READ_HEAD),
	read_buf_(rbuf_size), write_buf_(wbuf_size)
{
	timer_.data = this;
}
...
int CService::processHead(const char* p, uint32_t len)
{
	return -1;
}

int CService::processPkg(const char* p, uint32_t len)
{
	return -1;
}
...
void CService::read_cb(EV_P_ ev_io* w, int events)
{
	...
	{
		int ret = read(fd_, (void*)(read_buf_.p_ + read_buf_.tail_), len - read_buf_.tail_);
		if(ret > 0)
		{
			read_buf_.tail_ += ret;
...
bool CService::connect(string ip, uint16_t port, ev_tstamp timeout)
{
	if(fd_ < 0)
	{
		int type = timeout > 0. ? SOCK_STREAM|SOCK_NONBLOCK|SOCK_CLOEXEC : SOCK_STREAM|SOCK_CLOEXEC;
