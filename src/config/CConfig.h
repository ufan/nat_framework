...
class CConfig {
 ...
 public:
  ...
  void vector_as_T(const vector<string> &ori, vector<T> &res) {
    res.clear();
    for (vector<string>::const_iterator iter = ori.begin(); iter != ori.end();
         ++iter) {
  ...
  void getList(string block, string key, vector<T> &res, string delim = ",") {
    ...
    boost::split(destination, source, boost::is_any_of(delim),
...
template <>
inline void CConfig::vector_as_T<string>(const vector<string> &ori,
                                         vector<string> &res) {
  res = ori;
}
...
inline bool CConfig::initConfStr(string &config_content) {
  ...
  while (getline(in, line)) {
    ...
    // deal comment: ignore lines begin with '#'
    ...
    } else  // deal key-value, support key=val,
            // or key=val1 \n =val2 \n =val3 or key = val1 \n val2 \n val3
            // for list.
    {
      string val;
