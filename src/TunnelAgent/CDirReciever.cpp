...
CDirReciever::CDirReciever(CWaiter *p_owner) : CCommanderBase(p_owner), is_first_pkg_(true)
{
...
bool CDirReciever::writeFile()
{
	...
	int fd = open(name.c_str(), O_CREAT|O_TRUNC|O_CLOEXEC|O_RDWR, file_st.st_mode);
	if(fd < 0)
	{
