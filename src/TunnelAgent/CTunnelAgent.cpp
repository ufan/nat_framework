...
bool CTunnelAgent::init(string cfg_path)
{
	...
	if(-1 == (listen_fd_ = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0)))
	{
    ...
    // set sd reuseful
    int reuseaddr = 1;
    if (setsockopt(listen_fd_, SOL_SOCKET, SO_REUSEADDR, (const char*) &reuseaddr, sizeof(reuseaddr)) != 0)
    {
...
void CTunnelAgent::run()
{
    ...
    ev_run (loop, 0);		// loop forever, so the above stack variables is safe to use until exist.
...
void CTunnelAgent::accept_cb(EV_P_ ev_io *w, int events)
{
	...
	int fd = accept4(CTunnelAgent::instance()->listen_fd_, (struct sockaddr *)&addr, &addrlen, SOCK_NONBLOCK | SOCK_CLOEXEC);
	if(fd < 0)
	{
