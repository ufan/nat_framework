...
std::set<string> CTradeBaseInfo::productToInstrSet(const vector<string> &product)
{
	std::set<string> res;
	for(auto &i : product)
	{
...
std::set<string> CTradeBaseInfo::productOrInstrumentToInstrSet(const vector<string> &product)
{
	std::set<string> res;
	for(auto &i : product)
	{
...
void CTradeBaseInfo::callOnSwitchDayCb()
{
    switch_day_cb_(trading_day_);
}
...
{
	if(exch == "SSE") return EXCHANGEID_SSE;
	else if(exch == "SZE") return EXCHANGEID_SZE;
	else if(exch == "CFFEX") return EXCHANGEID_CFFEX;
	else if(exch == "SHFE") return EXCHANGEID_SHFE;
	else if(exch == "DCE") return EXCHANGEID_DCE;
	else if(exch == "CZCE") return EXCHANGEID_CZCE;
	else if(exch == "INE") return EXCHANGEID_INE;
	else if(exch == "SGE") return EXCHANGEID_SGE;
	return -1;
}
...
const char* exchangeint2str(int exch)
{
	switch(exch)
	{
	case EXCHANGEID_SSE: return "SSE";
	case EXCHANGEID_SZE: return "SZE";
	case EXCHANGEID_CFFEX: return "CFFEX";
	case EXCHANGEID_SHFE: return "SHFE";
	case EXCHANGEID_DCE: return "DCE";
	case EXCHANGEID_CZCE: return "CZCE";
	case EXCHANGEID_INE: return "INE";
	case EXCHANGEID_SGE: return "SGE";
	}
	return nullptr;
}
