...
inline double getCumTurnover(const OldUnitedMarketData *p)  {return p->cum_turnover;}
...
inline double getOpenInt(const OldUnitedMarketData *p)  {return p->open_interest;}
inline const char* getInstrStr(const OldUnitedMarketData *p) {return p->instr_str;}
