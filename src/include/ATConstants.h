...
enum class emOrderRtnType {
  ...
  CXLING = (1 << 3),  // order cancel request sent out by TDEngine or issued by
                      // TDHelper
  ACCEPT = (1 << 4),  // order insert/action accepted by Thost
  REJECT = (1 << 5) | 1,     // order insert rejected by Thost
  MARKET_ACCEPT = (1 << 6),  // order accepted by Exchange, waiting in the queue
