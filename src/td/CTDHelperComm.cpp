...
void CTDHelperComm::doSendOrder(int track_id)
{
	...
	tIOInputOrderField *p = (tIOInputOrderField*)td_writer_.prefetch(sizeof(tIOInputOrderField));
	p->cmd = IO_SEND_ORDER;
...
{
	tOrderTrack& order = getOrderTrack(track_id);
	tIOrderAction *p = (tIOrderAction*)td_writer_.prefetch(sizeof(tIOrderAction));
...
{
	return sysRequest(IO_TD_ADD_CLIENT, IO_TD_ACK_ADD_CLIENT, td_engine_id_, self_id_, timeout_, tdsend_path_.c_str(), tdsend_path_.size() + 1).size();
}
...
{
	string res = sysRequest(IO_TD_REQ_BASE_INFO, IO_TD_RSP_BASE_INFO, td_engine_id_, self_id_, timeout_);
	if(res.size())
	{
...
{
	string res = sysRequest(IO_TD_REQ_ORDER_TRACK, IO_TD_RSP_ORDER_TRACK, td_engine_id_, self_id_, timeout_);
	if(res.size())
	{
