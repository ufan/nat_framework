...
COrderTrackMmap::~COrderTrackMmap() { unload(); }
...
bool COrderTrackMmap::load(string name, bool is_write) {
  ...
  if (is_write) {
    ...
    // It has the effect of limiting the mmap file writable only to current
    // process if success.
    // This indicates only one running TD engine with the same name should
    // persist in a system.
  ...
  // Create memory-mapped file into buffer, this buffer is shared between
  // process
  buf_ = (tOrderTrackMmap*)mmap(
      0, size, (is_write) ? (PROT_READ | PROT_WRITE) : PROT_READ, MAP_SHARED,
      fd_, 0);
  ...
  if (is_lock_ &&
      (madvise(buf_, size, MADV_SEQUENTIAL) != 0 || mlock(buf_, size) != 0)) {
    // munmap(buf_, size);
    // close(fd_); fd_ = -1;
