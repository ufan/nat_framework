...
void CTDEngineCtp::connect(long timeout_nsec, string trade_flow_path) {
  for (int idx = 0; idx < account_units_.size(); idx++) {
    ...
    if (!unit.connected) {
      ...
      // wait for each connection result
      long start_time = CTimer::instance().getNano();
      while (!unit.connected &&
             CTimer::instance().getNano() - start_time < timeout_nsec) {
        usleep(50000);
      }
    }
  }
  ...
  for (int idx = 0; idx < account_units_.size(); idx++) {
    ...
    if (!unit.authenticated) {
      ...
      strcpy(reqAuth.UserProductInfo,
             unit.user_product_info.c_str());  // it's not necessary
      ...
      if (unit.api->ReqAuthenticate(&reqAuth, request_id_++)) {
        ALERT("[request] authenticate failed! (Bid)%s (Uid)%s",
              reqAuth.BrokerID, reqAuth.UserID);
      }

      // wait for response until timeout
      long start_time = CTimer::instance().getNano();
      while (!unit.authenticated &&
             CTimer::instance().getNano() - start_time < timeout_nsec) {
        usleep(50000);
      }
    }
    ...
    if (!unit.logged_in) {
      ...
      if (unit.api->ReqUserLogin(&req, request_id_++)) {
        ALERT("[request] login failed! (Bid)%s (Uid)%s", req.BrokerID,
              req.UserID);
      }

      // wait for login response until timeout
      long start_time = CTimer::instance().getNano();
      while (!unit.logged_in &&
             CTimer::instance().getNano() - start_time < timeout_nsec) {
        usleep(50000);
      }
    }

    // confirm settlement without checking first!!
    if (!unit.settle_confirmed) {
      ...
      if (unit.api->ReqSettlementInfoConfirm(&req, request_id_++)) {
        ALERT("[request] settlement info failed! (Bid)%s (Iid)%s", req.BrokerID,
              req.InvestorID);
      }

      // wait for settlement confirm until timeout
      long start_time = CTimer::instance().getNano();
      while (!unit.settle_confirmed &&
             CTimer::instance().getNano() - start_time < timeout_nsec) {
        usleep(50000);
      }
    }
  ...
  for (int idx = 0; idx < account_units_.size(); idx++) {
    ...
    if (unit.logged_in) {
      ...
      if (unit.api->ReqUserLogout(&req, request_id_++)) {
        ALERT("[request] logout failed! (Bid)%s (Uid)%s", req.BrokerID,
              req.UserID);
      }
    }
    unit.authenticated = false;
...
// Sync action, wait until all info is back.
bool CTDEngineCtp::queryInstruments(long timeout_nsec) {
  int ret = 0;
  CThostFtdcQryInstrumentField req;
  // empty field means query all instrument information
  memset(&req, 0, sizeof(req));
  // All accounts share the same set of instrument information, so only need to
  // query once
  if (0 !=
      (ret = account_units_[0].api->ReqQryInstrument(&req, request_id_++))) {
...
bool CTDEngineCtp::is_connected() const {
  for (auto& unit : account_units_) {
    if (!unit.connected) return false;
  }
  return true;
}
...
void CTDEngineCtp::OnRspAuthenticate(
  ...
  if (pRspInfo != nullptr && pRspInfo->ErrorID != 0) {
    ALERT("[OnRspAuthenticate] (errId)%d (errMsg)%s", pRspInfo->ErrorID,
          CEncodeConv::gbk2utf8(pRspInfo->ErrorMsg).c_str());
  } else {
    // print out authenticate response
    ENGLOG(
        "[OnRspAuthenticate] (Bid)%s (Uid)%s (AppId)%s (AppType)%d (ProInfo)%s",
        pRspAuthenticateField->BrokerID, pRspAuthenticateField->UserID,
...
void CTDEngineCtp::OnRspUserLogin(CThostFtdcRspUserLoginField* pRspUserLogin,
  ...
  if (pRspInfo != nullptr && pRspInfo->ErrorID != 0) {
    ALERT("[OnRspUserLogin] (errId)%d (errMsg)%s", pRspInfo->ErrorID,
          CEncodeConv::gbk2utf8(pRspInfo->ErrorMsg).c_str());
  } else  // User Login Succeed!
  {
    ENGLOG("[OnRspUserLogin] (Bid)%s (Uid)%s (maxRef)%s (Fid)%d (Sid)%d",
           pRspUserLogin->BrokerID, pRspUserLogin->UserID,
           pRspUserLogin->MaxOrderRef, pRspUserLogin->FrontID,
    ...
    for (auto& unit : account_units_) {
      if (unit.login_rid == nRequestID) {
        ...
        unit.action_order.SessionID = unit.session_id =
            pRspUserLogin->SessionID;
        unit.logged_in = true;
      }
    }
...
void CTDEngineCtp::OnRspSettlementInfoConfirm(
  ...
  // check whether the request is successful
  if (pRspInfo != nullptr && pRspInfo->ErrorID != 0) {
    ALERT("[OnRspSettlementInfoConfirm] (errId)%d (errMsg)%s",
          pRspInfo->ErrorID, CEncodeConv::gbk2utf8(pRspInfo->ErrorMsg).c_str());
  } else  // settlement confirmed successfully
  {
    ENGLOG(
        "[OnRspSettlementInfoConfirm] (brokerID)%s (investorID)%s "
...
void CTDEngineCtp::OnRspUserLogout(CThostFtdcUserLogoutField* pUserLogout,
                                   CThostFtdcRspInfoField* pRspInfo,
                                   int nRequestID, bool bIsLast) {
  if (unlikely(!checkRequestId(nRequestID))) return;

  if (pRspInfo != nullptr && pRspInfo->ErrorID == 0) {
    ALERT("[OnRspUserLogout] (errId)%d (errMsg)%s", pRspInfo->ErrorID,
          CEncodeConv::gbk2utf8(pRspInfo->ErrorMsg).c_str());
  } else {
    ENGLOG("[OnRspUserLogout] (brokerId)%s (userId)%s", pUserLogout->BrokerID,
           pUserLogout->UserID);
    for (auto& unit : account_units_) {
      if (unit.login_rid == nRequestID) {
        unit.logged_in = false;
...
void CTDEngineCtp::OnRspQryInstrument(CThostFtdcInstrumentField* pInstrument,
                                      CThostFtdcRspInfoField* pRspInfo,
                                      int nRequestID, bool bIsLast) {
  if (unlikely(!checkRequestId(nRequestID))) return;
  if (pRspInfo && pRspInfo->ErrorID != 0) {
    string err = CEncodeConv::gbk2utf8(pRspInfo->ErrorMsg);
    ALERT("CTP OnRspQryInstrument err:%d, msg:%s", pRspInfo->ErrorID,
          err.c_str());
  } else {
    if (pInstrument) {
      ...
      memcpy(info.expire_date, pInstrument->ExpireDate,
             sizeof(info.expire_date));
...
void CTDEngineCtp::OnRspOrderInsert(CThostFtdcInputOrderField* pInputOrder,
  ...
  if (pRspInfo && request_track.from != 0 && CHK_USERID(pInputOrder)) {
    ...
    memcpy(p->rtn_msg.instr, pInputOrder->InstrumentID,
           sizeof(pInputOrder->InstrumentID));
...
void CTDEngineCtp::OnRspOrderAction(
  ...
  if (pRspInfo && request_track.from != 0 && CHK_USERID(pInputOrderAction)) {
    ...
    p->rtn_msg.local_id = request_track.local_id;
    memcpy(p->rtn_msg.instr, pInputOrderAction->InstrumentID,
           sizeof(pInputOrderAction->InstrumentID));
    p->rtn_msg.instr_hash = INSTR_NAME_TO_HASH(p->rtn_msg.instr);
    p->rtn_msg.price = request_track.price;
    p->rtn_msg.vol = request_track.vol;
    p->rtn_msg.dir = request_track.dir;
...
void CTDEngineCtp::OnRtnTrade(CThostFtdcTradeField* pTrade) {
  ...
  if (request_track.from != 0) {
    ...
    if (request_track.vol_traded >= request_track.vol)
      request_track.status |= ODS(CLOSED);
    ...
    p->rtn_msg.local_id = request_track.local_id;
    memcpy(p->rtn_msg.instr, pTrade->InstrumentID,
           sizeof(pTrade->InstrumentID));
    p->rtn_msg.instr_hash = INSTR_NAME_TO_HASH(p->rtn_msg.instr);
...
void CTDEngineCtp::OnErrRtnOrderAction(CThostFtdcOrderActionField* pOrderAction,
  ...
  if (request_track.from != 0) {
    ...
    p->rtn_msg.msg_type = ODS(CANCEL_REJECT);
    p->rtn_msg.local_id = request_track.local_id;
    memcpy(p->rtn_msg.instr, pOrderAction->InstrumentID,
           sizeof(pOrderAction->InstrumentID));
    p->rtn_msg.instr_hash = INSTR_NAME_TO_HASH(p->rtn_msg.instr);
    p->rtn_msg.price = request_track.price;
    p->rtn_msg.vol = request_track.vol;
    p->rtn_msg.dir = request_track.dir;
  ...
  if ((uint32_t)(data->acc_idx) < acc_utilis_.size()) {
    ...
    if (unlikely(ret = util->check(data->dir, off, data->price, data->vol,
      ...
      int2string(order.OrderRef,
                 request_id_);  // order_ref comes from current request_id_
      ...
      if (ret == 0) {
        ...
        // LOG_DBG("elapse:%ld,%ld,%ld,%ld", start_order -
...
void CTDEngineCtp::req_order_action(const tIOrderAction* data) {
  if (unlikely(!checkRequestId(data->order_ref))) {
    writeErrRtn(data, -1,
                "cancel order reject by TDEngine: order_ref range err.",
                ODS(CANCEL_REJECT));
    return;
  }
  ...
  if (request_track.front_id == data->front_id &&
    ...
    int ret = unit.api->ReqOrderAction(
        &order, data->order_ref);  // use order_ref as request_id
    if (ret == 0) {
      request_track.status |= ODS(CXLING);
    } else {
      writeErrRtn(data, ret, "ctp ReqOrderAction api failed.",
                  ODS(CANCEL_REJECT));
    }
  } else {
    writeErrRtn(data, -1, "cancel order reject by TDEngine.",
                ODS(CANCEL_REJECT));
  }
}
...
bool CTDEngineCtp::updateOrderTrack() {
  ...
  for (int idx = 0; idx < account_units_.size(); idx++) {
    ...
    long start_time = CTimer::instance().getNano();
    while (!query_complete_flag_ &&
           CTimer::instance().getNano() - start_time < timout_ns_) {
      usleep(50000);
    }
...
void CTDEngineCtp::OnRspQryOrder(CThostFtdcOrderField* pOrder,
                                 CThostFtdcRspInfoField* pRspInfo,
                                 int nRequestID, bool bIsLast) {
  if (unlikely(!checkRequestId(nRequestID))) return;
  ...
  if (pOrder) {
    ...
    // track.off = pOrder->CombOffsetFlag[0];			// comment out
    // this line, because this field from ctp may lose some information.
...
void CTDEngineCtp::OnRspQryTrade(CThostFtdcTradeField* pTrade,
                                 CThostFtdcRspInfoField* pRspInfo,
                                 int nRequestID, bool bIsLast) {
  if (unlikely(!checkRequestId(nRequestID))) return;
  if (bIsLast) query_complete_flag_ = true;
...
CTDEngineCtp* createTDEngineCTP() { return new CTDEngineCtp; }
