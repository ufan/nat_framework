...
void CTDEngineEES::connect()
{
    ...
    int query_port = config_["/EESTD/query_port"_json_pointer];
    int trade_port = config_["/EESTD/trade_port"_json_pointer];
    ...
    for (int idx = 0; idx < account_units_.size(); idx ++)
    {
        ...
        {
            curAccountIdx_ = idx;
            RESULT res = unit.api->ConnServer(trade_ip.c_str(), trade_port, this, query_ip.c_str(), query_port);
            if (res != NO_ERROR)
            {
...
void CTDEngineEES::OnOrderMarketAccept(EES_OrderMarketAcceptField* pAccept)
{
    ...
    for(int i = request_id_ - 1; i >= request_id_start_; i--)
    {
        ...
        if(request_track.session_id && request_track.front_id == pAccept->m_MarketOrderToken)
        {
    ...
    if(p_track)
    {
    	if(p_track->status & ODS(MARKET_ACCEPT)) return;

        p_track->status |= ODS(MARKET_ACCEPT);
        p_track->front_id = pAccept->m_MarketOrderToken;
    ...
    {
        LOG_ERR("OnOrderMarketAccept: cannot find order track match MarketToken:%lld", pAccept->m_MarketOrderToken);
    }
...
void CTDEngineEES::OnOrderMarketReject(EES_OrderMarketRejectField* pReject)
{
    ...
    if(p_track)
    {
    	if(p_track->status & ODS(MARKET_REJECT)) return;
    ...
    {
        LOG_ERR("OnOrderMarketReject: cannot find order track match MarketToken:%lld", pReject->m_MarketOrderToken);
    }
}
