...
/**
 * @brief Authentication and certification with the server using private key
 * @details 1. Send authentication to the server, wait for the reply.
            2. Authenticate the server using the reply msg.
 * @return Description
 */
bool CClient::sslAuthen() {
  ...
  // authen myself using private key
  ...
  // read reply msg and authen server
...
/**
 * @brief Generate a new 3DES key and distribute to the server using RSA private
 * key
 */
bool CClient::sendDesKey() {
  ...
  tMetaHead head = {META_HEAD_STX, META_HEAD_VER, 0, uint32_t(encrypt.size())};
...
/**
 * @brief Send a list commands to server.
...
bool CClient::doOneCommand(string cmd) {
  ...
  if (strcasecmp(order.c_str(), "SAVE") == 0) {
    ...
    return doExecCmd(cmd_split[1], cmd_split[2],
                     joinCmdVector(cmd_split, " ", 3));
  } else if (strcasecmp(order.c_str(), "EXEC_DIR") == 0) {
    if (cmd_split.size() < 4) return false;  // EXEC_DIR workdir config exe args
    return doExecCmd(cmd_split[2], cmd_split[3],
                     joinCmdVector(cmd_split, " ", 4), cmd_split[1]);
...
bool CClient::desRead(string &pkg) {
  ...
  if (ret <= 0) {
    if (ret == 0)
      LOG_ERR("read err:closed by peer.");
    else
      LOG_ERR("read err:%s", strerror(errno));
    return false;
  }
...
bool CClient::sendDir(string localdir, string remotedir, uint8_t cmd,
  ...
  if (remotedir.empty())
    return true;  // if remote dirname is empty, just return true.
