...
CRawIOWriter::CRawIOWriter(uint32_t page_size)
    : page_num_(-1),
      prefetch_tail_(0),
      page_size_(page_size),
      is_test_lock_onload_(
          true)  // try to lock the file on load, fail if can't lock
{}

CRawIOWriter::~CRawIOWriter() {}

// Create next Page file.
// 'Next' means increment the page_num_ by 1 and use the new number as file
// suffix
...
// If this file is already finished, increment the Page_number by 1 and create a
// new mmap Page file.
...
// TODO: add checking of the status code!
...
// Instead of writing the data into new frame directly, fetch the empty frame
// first. 'len' indicates the size of the frame data bytes to be filled later.
// If the Page's space is limited, the current one will be closed and a new one
// created. 'prefetch_tail' will be set to the expected length of the valid data
// in the Page including the prefetched frame. The returned pointer can be used
// by the user to fill in the frame bytes later, directly.
...
// Used together with 'prefetch', the step where the frame data bytes are
// filled. The current timestamp will be used as the newly-filled frame time.
...
bool CRawIOWriter::load(int num) {
  ...
  // It's newly created set to default size, if not new, set the the actual size
  // of file The file is regarded as newly-created as long as it's size is
  // smaller than the size of PageHead.
  bool isnew = false;
  ...
  // Check the opened file is the correct mmap file by checking the buffer
  // header, if the file is not newly-created.
  ...
  // Try to avoid the page-out of the mapped-memory, i.e. lock the pages in
  // physical memory
  if (is_lock_ &&
      (madvise(buf_, size_, MADV_SEQUENTIAL) != 0 || mlock(buf_, size_) != 0)) {
    // munmap(buf_, size_);
    // close(fd_); fd_ = -1;
...
void CSafeRawIOWriter::discard() { g_io_spin_flag.clear(memory_order_release); }
