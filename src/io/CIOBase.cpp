...
CIOBase::CIOBase(uint32_t page_size)
    : buf_(NULL), size_(page_size), fd_(-1), is_lock_(true) {}

CIOBase::~CIOBase() { unload(); }
...
/**
 * @brief Init a mmap file
 * @details If opened for read and the page size equals the mmap file.
 *          If opened for write and the mmap file already exist the file's size
 *          will be truncated. Also, the mmap file will be locked for the
 *          calling process if opened for write.
 * @param[in] num file name index number
 * @param[in] is_write open the mmap file for read or write
 * @return true on success, false on fail
 */
bool CIOBase::load(int num, bool is_write) {
  string path = prefix_ + to_string(num);

  fd_ = open(path.c_str(), (is_write) ? (O_RDWR | O_CREAT) : O_RDONLY,
             (mode_t)0666);
  if (fd_ < 0) {
  ...
  buf_ =
      (char *)mmap(0, size_, (is_write) ? (PROT_READ | PROT_WRITE) : PROT_READ,
                   MAP_SHARED, fd_, 0);
