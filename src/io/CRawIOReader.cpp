...
bool CRawIOReader::init(string path, long from_nano) {
  setPrefix(path);
  if (from_nano < 0)
    return init(path, -1, from_nano);
  else
    return init(path, getHeadNum(), from_nano);
}
...
// if frame_no < 0 or out of current range, it means moving to the tail of
// current Page
...
inline bool CRawIOReader::passPage() {
  ...
  } else if (((volatile tPageHead* volatile)buf_)->status ==
             PAGE_STATUS_FINISH) {
    unload();
    return load(++page_num_);
  }
...
//
// Internal use in class, can't handle the situation when nano is very small
// even for current Page, since it only look forward with larger page_num. Use
// setReadPos to correctly find the frame in all situations, including looking
// backward with smaller page_num.
...
bool CRawIOReader::load(int num) {
  ...
  if (fd_ < 0) {
    if (unlikely(errno != ENOENT)) {
      LOG_ERR("Cannot open file %s, err %d:%s", path.c_str(), errno,
              strerror(errno));
    }
    return false;
  }
  ...
  if (fstat(fd_, &statbuff) < 0) {
    close(fd_);
    fd_ = -1;
    LOG_ERR("stat file %s fail, err %d:%s", path.c_str(), errno,
            strerror(errno));
    return false;
  }
