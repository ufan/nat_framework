...
bool isMine(const char* p, uint32_t len, uint32_t stg_hash) {
  tSysIOHead* t = (tSysIOHead*)p;
  if (len < sizeof(tSysIOHead)) {
    return true;
  } else {
    if (t->to == stg_hash || t->source == stg_hash) {
      return true;
    }
  }
  return false;
...
void printSysIOHead(const char* p, uint32_t len) {
  tSysIOHead* t = (tSysIOHead*)p;
  printf("[cmd]%s [to]%d [source]%d [back_word]%d", getCmdString(t->cmd),
...
void printIOMarketData(const char* p, uint32_t len) {
  ...
  printf(
      "[cmd]%s [instr_hash]%u [last_px]%.2f [cum_vol]%d [cum_turnover]%.2lf [avg_px]%.2f \
...
[ask_px5]%.2f [bid_px5]%.2f [ask_vol5]%d [bid_vol5]%d [open_interest]%.2lf [exch_time]%ld [instr_str]%s",
      getCmdString(t->cmd), t->market_data.instr_hash, t->market_data.last_px,
      t->market_data.cum_vol, t->market_data.cum_turnover,
      t->market_data.avg_px, t->market_data.ask_px, t->market_data.bid_px,
      t->market_data.ask_vol, t->market_data.bid_vol, t->market_data.ask_px2,
      t->market_data.bid_px2, t->market_data.ask_vol2, t->market_data.bid_vol2,
      t->market_data.ask_px3, t->market_data.bid_px3, t->market_data.ask_vol3,
      t->market_data.bid_vol3, t->market_data.ask_px4, t->market_data.bid_px4,
      t->market_data.ask_vol4, t->market_data.bid_vol4, t->market_data.ask_px5,
      t->market_data.bid_px5, t->market_data.ask_vol5, t->market_data.bid_vol5,
      t->market_data.open_interest, t->market_data.exch_time,
...
void printIOInputOrder(const char* p, uint32_t len) {
  ...
  printf(
      "[cmd]%s [extra_nano]%ld [from]%d [local_id]%d [acc_idx]%d "
      "[instr_hash]%u [instr]%s [price]%.2lf [vol]%d [dir]%s [off]%s",
      getCmdString(t->cmd), t->extra_nano, t->from, t->local_id, t->acc_idx,
      t->instr_hash, t->instr, t->price, t->vol, getDirString(t->dir),
      getOffString(t->off));
...
void printIOrderAction(const char* p, uint32_t len) {
  ...
  printf(
      "[cmd]%s [from]%d [local_id]%d [acc_idx]%d [order_ref]%ld [front_id]%ld "
      "[session_id]%ld [instr_hash]%u [instr]%s",
      getCmdString(t->cmd), t->from, t->local_id, t->acc_idx, t->order_ref,
      t->front_id, t->session_id, t->instr_hash, t->instr);
...
void printIOrderRtn(const char* p, uint32_t len) {
  ...
  printf(
      "[cmd]%s [to]%d [msg_type]%s [local_id]%d [instr_hash]%u [instr]%s [price]%.2lf [vol]%d [dir]%s [off]%s \
[order_ref]%ld [front_id]%ld [session_id]%ld [errid]%d [msg]%s",
      getCmdString(t->cmd), t->to,
      getEmOrderRtnTypeString(t->rtn_msg.msg_type).c_str(), t->rtn_msg.local_id,
      t->rtn_msg.instr_hash, t->rtn_msg.instr, t->rtn_msg.price, t->rtn_msg.vol,
      getDirString(t->rtn_msg.dir), getOffString(t->rtn_msg.off),
      t->rtn_msg.order_ref, t->rtn_msg.front_id, t->rtn_msg.session_id,
      t->rtn_msg.errid, t->rtn_msg.msg);
...
void printIOTDBaseInfo(const char* p, uint32_t len) {
  ...
  printf("[trading_day]%s [instr_cnt]%d ", t->trading_day, t->instr_cnt);

  printf("instrument_info:{");
  for (int i = 0; i < t->instr_cnt; ++i) {
    tInstrumentInfo* ot = (tInstrumentInfo*)t->instr;
    printf(
        "(%d/%d)[instr_hash:%u;instr:%s;exch:%d;product:%s;product_hash:%u;vol_multiple:%d;\
tick_price:%.2lf;expire_date:%s;is_trading:%d],",
        i + 1, t->instr_cnt, ot->instr_hash, ot->instr, ot->exch, ot->product,
        ot->product_hash, ot->vol_multiple, ot->tick_price, ot->expire_date,
        ot->is_trading);
    ++ot;
  }
  printf("};");

  size_t siz = sizeof(tSysIOHead) + sizeof(tIOTDBaseInfo) +
               sizeof(tInstrumentInfo) * t->instr_cnt;
  if (len > siz) {
    printBuf(p + siz, len - siz);
  }
}
...
void printIOMArketTradingDay(const char* p, uint32_t len) {
  ...
  printf("[cmd]%s [trading_day]%s [day_night_mode]%s", getCmdString(t->cmd),
         t->trading_day, t->day_night_mode);
...
void printIOrderTrack(const char* p, uint32_t len) {
  ...
  printf("[cmd]%s [to]%d [source]%d [back_word]%d [cnt]%d",
         getCmdString(t->cmd), t->to, t->source, t->back_word, t->cnt);

  printf("order_track:{");
  for (int i = 0; i < t->cnt; ++i) {
    tOrderTrack* ot = (tOrderTrack*)t->track;
    printf(
        "(%d/%d)[[status]%s [instr_hash]%u [instr]%s [price]%.2lf [vol]%d [dir]%s [off]%s [vol_traded]%d \
[amount_traded]%.2lf [from]%d [local_id]%d [acc_id]%d [stg_id]%d [order_ref]%ld [front_id]%ld [session_id]%ld]],",
        i + 1, t->cnt, getEmOrderRtnTypeString(ot->status).c_str(),
        ot->instr_hash, ot->instr, ot->price, ot->vol, getDirString(ot->dir),
        getOffString(ot->off), ot->vol_traded, ot->amount_traded, ot->from,
        ot->local_id, ot->acc_id, ot->stg_id, ot->order_ref, ot->front_id,
        ot->session_id);
    ++ot;
  }
  printf("};");
  ...
  while (true) {
    ...
    if (p) {
      ...
      if (from_nano <= nano && nano <= to_nano) {
        ...
        printf("[time_str]%s [time]%ld [length]%u [file]%s ", t.c_str(), nano,
               len, map_hash_file[hash].c_str());
        printCmdContent(p, len);
        printf("\n");
      }
    } else {
      usleep(50000);  // exit when no data coming in 50s
      break;
    }
  }
}
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    printf("Usage: %s {mmap_file} [from_nano] [to_nano]\n", argv[0]);
    return -1;
  } else {
    char file[256] = {0};
    int from_no = 0;
    long from_nano = 0;
    long to_nano = LONG_MAX;

    if (argc >= 2) {
      strcpy(file, argv[1]);
      char* p_needle = nullptr;
      if ((p_needle = strstr(file, "_io_")) != nullptr) {
        *p_needle = '\0';
      }
    }

    if (argc >= 3) {
      from_nano = atol(argv[2]);
    }
    if (argc >= 4) {
      to_nano = atol(argv[3]);
    }

    readIO(file, from_no, from_nano, to_nano);
  }
