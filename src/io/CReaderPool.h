...
class CReaderPool {
 ...
 public:
  ...
  // Adding a reader with id 'hash' using 'path', 'from_page' and 'from_nano' as
  // filter
  ...
  // Iterate all reader's next frame, and read the one with earliest timestamp
  // Return null if no new frame availbale.
  // Return the pointer to the frame data buffer, buffer size and reader id
