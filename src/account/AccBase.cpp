...
bool AccBase::regInstr(const char* instr_name) {
  // 1. get base info of the instrument
  ...
  // 2. insert a new record if non-exist
  ...
  if (!isExistModInstr(instr_hash)) {
    ...
    // add the instrument to the corresponding product summary and this
    // account's summary
...
bool AccBase::getJsonFilePath(int t, string day, string& path) {
  LOG_INFO("%s(%d, %s, %s), local_time: %ld, trading_day: %s", __FUNCTION__, t,
           day.c_str(), path.c_str(), CTimer::instance().getNano(),
           CTradeBaseInfo::trading_day_.c_str());
...
json AccBase::to_json() {
  LOG_INFO("%s(), local_time: %ld, trading_day: %s", __FUNCTION__,
           CTimer::instance().getNano(), CTradeBaseInfo::trading_day_.c_str());
  ...
  for (auto it = map_prd_.begin(); it != map_prd_.end(); ++it) {
    ...
    if (0 != unit_vol.pos_long_yd_ini || 0 != unit_vol.pos_long_yd_cls_ing ||
        0 != unit_vol.pos_long_td_opn_ing ||
        0 != unit_vol.pos_long_td_cls_ing || 0 != unit_vol.pos_long ||
        0 != unit_vol.pos_short_yd_ini || 0 != unit_vol.pos_short_yd_cls_ing ||
        0 != unit_vol.pos_short_td_opn_ing ||
        0 != unit_vol.pos_short_td_cls_ing || 0 != unit_vol.pos_short) {
...
bool AccBase::from_json(json& j) {
  LOG_INFO("%s(json), local_time: %ld, trading_day: %s", __FUNCTION__,
           CTimer::instance().getNano(), CTradeBaseInfo::trading_day_.c_str());
  ...
  for (json::iterator it = j["mod_instr"].begin(); it != j["mod_instr"].end();
       ++it) {
  ...
  for (json::iterator it = j["mod_prd"].begin(); it != j["mod_prd"].end();
       ++it) {
...
bool AccBase::save(string day, bool force) {
  LOG_INFO("%s(%s, %d), local_time: %ld, trading_day: %s", __FUNCTION__,
           day.c_str(), force, CTimer::instance().getNano(),
           CTradeBaseInfo::trading_day_.c_str());

  if (!force && j_conf_.find("save_account_daily") != j_conf_.end() &&
      j_conf_["save_account_daily"].get<bool>() == false) {
    return true;
  }
...
bool AccBase::load() {
  ...
  LOG_INFO("%s(), local_time: %ld, trading_day: %s, load acc file: %s",
           __FUNCTION__, CTimer::instance().getNano(),
           CTradeBaseInfo::trading_day_.c_str(), path.c_str());
...
void AccBase::onOrderTrack(const tOrderTrack* p_ord_trk) {
  ...
  if (p_ord_trk->status & (ODS(SEND) | ODS(TDSEND))) {
    LOG_DBG(
        "onOrderTrack, New, [order_ref]%ld, [dir]%s, [off]%s, [px]%.2lf, "
        "[vol]%d",
        p_ord_trk->order_ref, getDirString(p_ord_trk->dir),
        getOffString(p_ord_trk->off), p_ord_trk->price, p_ord_trk->vol);
    mod_instr.onNew(p_ord_trk->dir, p_ord_trk->off, p_ord_trk->price,
                    p_ord_trk->vol);
  }
  if ((p_ord_trk->status & ODS(CLOSED)) &&
      (p_ord_trk->vol != p_ord_trk->vol_traded)) {
    LOG_DBG(
        "onOrderTrack, Cxl, [order_ref]%ld, [dir]%s, [off]%s, [px]%.2lf, "
        "[vol]%d",
        p_ord_trk->order_ref, getDirString(p_ord_trk->dir),
        getOffString(p_ord_trk->off), p_ord_trk->price,
        p_ord_trk->vol - p_ord_trk->vol_traded);
    mod_instr.onCxl(p_ord_trk->dir, p_ord_trk->off, p_ord_trk->price,
                    p_ord_trk->vol - p_ord_trk->vol_traded);
  }
  if (p_ord_trk->status & ODS(EXECUTION)) {
    double trd_px = (0 == p_ord_trk->vol_traded)
                        ? 0.0
                        : p_ord_trk->amount_traded / p_ord_trk->vol_traded;
    LOG_DBG(
        "onOrderTrack, Trd, [order_ref]%ld, [dir]%s, [off]%s, [px]%.2lf, "
        "[vol]%d, [vol_traded]%d, [trd_px]%.2lf, [trd_vol]%d",
        p_ord_trk->order_ref, getDirString(p_ord_trk->dir),
        getOffString(p_ord_trk->off), p_ord_trk->price, p_ord_trk->vol,
        p_ord_trk->vol_traded, trd_px, p_ord_trk->vol_traded);
    mod_instr.onTrd(p_ord_trk->dir, p_ord_trk->off, p_ord_trk->price,
                    p_ord_trk->vol, p_ord_trk->vol_traded, trd_px,
                    p_ord_trk->vol_traded);
  }
}
...
void AccBase::onNew(int dir, int off, double px, int vol, uint32_t instr_hash) {
  ...
  LOG_DBG("onNew, [instr]%s, [dir]%s, [off]%s, [px]%.2lf, [vol]%d",
          p_instr_info->instr, getDirString(dir), getOffString(off), px, vol);
...
void AccBase::onRtn(const tOrderTrack* p_ord_trk, const tRtnMsg* p_rtn_msg) {
  ...
  if (p_rtn_msg->msg_type &
      (ODS(REJECT) | ODS(MARKET_REJECT) | ODS(CANCELED))) {
    LOG_DBG("onRtn, Cxl, [instr]%s, [dir]%s, [off]%s, [px]%.2lf, [vol]%d",
            p_ord_trk->instr, getDirString(p_ord_trk->dir),
            getOffString(p_ord_trk->off), p_ord_trk->price, p_rtn_msg->vol);
    mod_instr.onCxl(p_ord_trk->dir, p_ord_trk->off, p_ord_trk->price,
                    p_rtn_msg->vol);
  }
  if (p_rtn_msg->msg_type & ODS(EXECUTION)) {
    LOG_DBG(
        "onRtn, Trd, [instr]%s, [dir]%s, [off]%s, [px]%.2lf, [vol]%d, "
        "[vol_traded]%d, [trd_px]%.2lf, [trd_vol]%d",
        p_ord_trk->instr, getDirString(p_ord_trk->dir),
        getOffString(p_ord_trk->off), p_ord_trk->price, p_ord_trk->vol,
        p_ord_trk->vol_traded, p_rtn_msg->price, p_rtn_msg->vol);
    mod_instr.onTrd(p_ord_trk->dir, p_ord_trk->off, p_ord_trk->price,
                    p_ord_trk->vol, p_ord_trk->vol_traded, p_rtn_msg->price,
                    p_rtn_msg->vol);
  }
}
...
ModAcc* AccBase::getModAcc() { return &mod_acc_; }
...
UnitAmt* AccBase::getAccUnitAmt() { return &mod_acc_.unit_amt; }

UnitPnl* AccBase::getAccUnitPnl() { return &mod_acc_.unit_pnl; }
