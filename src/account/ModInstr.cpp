...
void ModInstr::onTickPx(double tick_px) {
  double delta = 0.0;
  delta = unit_amt.calcAmtChangeOnPxChange(tick_px, unit_px.last_px,
                                           unit_vol.pos_long, vol_multiple);
  ...
  delta = unit_amt.calcAmtChangeOnPxChange(tick_px, unit_px.last_px,
                                           unit_vol.pos_short, vol_multiple);
  ...
  delta = unit_pnl.calcPosPnlChangeOnPxChange(
      tick_px, unit_px.last_px, unit_vol.pos_long_yd_ini,
      unit_vol.pos_short_yd_ini, vol_multiple);
  ...
  delta = unit_pnl.calcTrdPnlChangeOnPxChange(tick_px, unit_px.last_px,
                                              unit_vol.pos_buy,
                                              unit_vol.pos_sell, vol_multiple);
...
/**
 * @brief Update the account when new order is issued
 * @details Description
 * @param[in] dir Buy/Sell
 * @param[in] off Open/CloseYesterday/CloseToday
 * @param[in] px Price
 * @param[in] vol Volume
 */
void ModInstr::onNew(int dir, int off, double px, int vol) {
  // change of amount with issued order parameter
  double amt_ing_chg =
      unit_amt.calcAmtIngChangeOnNewCxlTrd(px, vol, vol_multiple);

  // just issued, not filled yet
    ...
    case AT_CHAR_Buy:
      switch (off) {
        case AT_CHAR_Open:
          ...
          // update amount (long position increase)
        ...
        case AT_CHAR_CloseYesterday:
          ...
          // update amount (short position decrease)
    ...
    case AT_CHAR_Sell:
      switch (off) {
        case AT_CHAR_Open:
          ...
          // amount (short position increase)
...
void ModInstr::onCxl(int dir, int off, double px, int cxl_vol) {
  double amt_ing_chg =
      -1 * unit_amt.calcAmtIngChangeOnNewCxlTrd(px, cxl_vol, vol_multiple);
  --p_acc->unfilled_order_cnt;
  switch (dir) {
    case AT_CHAR_Buy:
      switch (off) {
...
void ModInstr::onTrd(int dir, int off, double px, int vol, int vol_traded,
                     double trd_px, int trd_vol) {
  onTickPx(trd_px);

  double amt_ing_chg =
      -1 * unit_amt.calcAmtIngChangeOnNewCxlTrd(px, trd_vol, vol_multiple);
  ...
  switch (dir) {
    case AT_CHAR_Buy:
      ...
      unit_px.avg_px_buy =
          unit_px.calcAvgPx(unit_amt.amt_buy, unit_vol.pos_buy, vol_multiple);
      break;
    case AT_CHAR_Sell:
      switch (off) {
        case AT_CHAR_Open:
      ...
      unit_px.avg_px_sell =
          unit_px.calcAvgPx(unit_amt.amt_sell, unit_vol.pos_sell, vol_multiple);
      break;
    default:
      ALERT("unknown dir=%d.", dir);
      break;
