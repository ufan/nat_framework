 ...
 *      Author: hongxu
 *
 * This executable will search for the latest trading day in the passed-in page
 * file and ask the user whether delete the data after this day (including this
 * day).
 * Only the data in this particular page file will be deleted. The page number
 * is not incremented autotmatically
 */
...
int main(int argc, char *argv[]) {
  ...
  if (lst_info) {
    if (nearest) {
      const char *p_day =
          ((tIOTDBaseInfo *)((tSysIOHead *)(lst_info->buf))->data)->trading_day;
      printf(
          "find nearest trading day at %s. data after(include) this day will "
          "be discarded. confirm[y|n]:",
          p_day);
      char c;
      scanf("%c", &c);
      if (c == 'y') {
