...
CMarketStruct::CMarketStruct(){
    ...
    std::cout << "CMarketStruct::Init" <<endl;
}
...
void CMarketStruct::OnTick(const UnitedMarketData *md){
    const tInstrumentInfo instr_info = CTradeBaseInfo::instr_info_[md->instr_hash];
    int exchange = instr_info.exch;
    
    double ticksize = instr_info.tick_price; //ticksize and multiplier placed as input from json
    int multiplier = instr_info.vol_multiple;
    
    bidp_[0] = int(round(md->bid_px/ticksize));
    bidv_[0] = md->bid_vol;
    bidp_[1] = int(round(md->bid_px2/ticksize));
    bidv_[1] = md->bid_vol2;
    bidp_[2] = int(round(md->bid_px3/ticksize));
    bidv_[2] = md->bid_vol3;
    bidp_[3] = int(round(md->bid_px4/ticksize));
    bidv_[3] = md->bid_vol4;
    bidp_[4] = int(round(md->bid_px5/ticksize));
    bidv_[4] = md->bid_vol5;
    
    askp_[0] = int(round(md->ask_px/ticksize));
    askv_[0] = md->ask_vol;
    askp_[1] = int(round(md->ask_px2/ticksize));
    askv_[1] = md->ask_vol2;
    askp_[2] = int(round(md->ask_px3/ticksize));
    askv_[2] = md->ask_vol3;
    askp_[3] = int(round(md->ask_px4/ticksize));
    askv_[3] = md->ask_vol4;
    askp_[4] = int(round(md->ask_px5/ticksize));
    askv_[4] = md->ask_vol5;
    ...
    if(is_last_tick_ready_){    
        ...
        turnover_ = int(round(turnover/(ticksize*multiplier)));
        if(exchange == EXCHANGEID_SHFE){
