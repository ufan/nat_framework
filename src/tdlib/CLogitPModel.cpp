...
CLogitPModel::CLogitPModel(){
    ...
    std::cout << "CLogitPModel::Init" <<endl;    
    std::ifstream f1("CLogitCoeff.cnf");
    if(!f1)
    ...
    for(int count=0; count< TOTAL_DELTA_; count++){
        ...
        IsProbSet1_[count] = param_reader["IsProbSet1"][count];       
    } 
...
double CLogitPModel::EvaluateProb(int level_index, int tradesize, const CBaseSignal& bs, double volatility, double timeleft, double duration, int direction){
    double prob = 1.0;
