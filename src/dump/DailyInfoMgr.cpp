...
bool DailyInfoMgr::write()
{
	...
	{
		string path = string(daily_info_path) + "/CTP_" + trading_date + "_daily.info";
...
void DailyInfoMgr::onTick(CThostFtdcDepthMarketDataField* pDepthMarketData)
{
	...
	if (pDepthMarketData->SettlementPrice != 0 && pDepthMarketData->SettlementPrice != DBL_MAX)
	{
		DailyInfoMgr::j["statistic"][string(pDepthMarketData->InstrumentID)]["settlement_price"] = pDepthMarketData->SettlementPrice;
		DailyInfoMgr::j["statistic"][string(pDepthMarketData->InstrumentID)]["open_interest"] = pDepthMarketData->OpenInterest;
		DailyInfoMgr::j["statistic"][string(pDepthMarketData->InstrumentID)]["cum_vol"] = pDepthMarketData->Volume;
		DailyInfoMgr::j["statistic"][string(pDepthMarketData->InstrumentID)]["cum_turnover"] = pDepthMarketData->Turnover;
		DailyInfoMgr::j["statistic"][string(pDepthMarketData->InstrumentID)]["tick_cnt"] = DailyInfoMgr::map_instr_cnt[string(pDepthMarketData->InstrumentID)];
	}
}