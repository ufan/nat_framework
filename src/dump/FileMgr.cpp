...
bool FileMgr::regInstrument(const char* trading_date, vector<Instrument>& vec_instr)
{
	map_instr_prd.clear();
	map_prd_file.clear();
	...
	{
		char dirname[256] = {0};
		snprintf(dirname, sizeof(dirname), "%s/China/%s/%s", warehouse, map_prd_exch[it->first].c_str(), it->first.c_str());
		...
		char filename[256] = {0};
		snprintf(filename, sizeof(filename), "%s/CTP_%s_%s_%s.data", dirname, trading_date, map_prd_exch[it->first].c_str(), it->first.c_str());
	...
	char filename[256] = {0};
	snprintf(filename, sizeof(filename), "%s/CTP_%s_All.data", dirname, trading_date);
	FILE* pf = fopen(filename, "ab");
	if (pf == nullptr)
	{
		ALERT("can't open file %s, errno:%d, errmsg:%s\n", filename, errno, strerror(errno));
