...
bool CCTPMD::init(json &j) {
  ...
  if (api == nullptr) {
    api =
        CThostFtdcMdApi::CreateFtdcMdApi(j["flow_path"].get<string>().c_str());
    if (!api) {
...
/**
 * @brief Subscribe a list of instruments
 */
...
/**
 * @brief Check if it's official trading time or not
 */
bool CCTPMD::isInTradingTime() {
  ...
  if ((3 * 3600 <= hrmise && hrmise < 8 * 3600) ||
      (16 * 3600 <= hrmise && hrmise < 20 * 3600)) {
    return false;
  } else {
    return true;
  }
...
void CCTPMD::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
...
void CCTPMD::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout,
                             CThostFtdcRspInfoField *pRspInfo, int nRequestID,
                             bool bIsLast) {
...
/**
 * @brief Receive the subscribed tick-quote data.
 *
 * Both the warehouse binary file and the daily_info JSON file are updated
 * accordingly. Each tick has a receiving timestamp (ns) as its header field in
 * the binary file.
 */
void CCTPMD::OnRtnDepthMarketData(
    CThostFtdcDepthMarketDataField *pDepthMarketData) {
  // the time when this tick is received, in ns (since epoch?).
  ...
  // update the statistics of the JSON file
