...
int CExecuteStrategy::execution(uint32_t instr_hash, double priceup,
                                double pricedown, int placedsize, int direction,
                                double duration) {
  auto itr = loss_model_.find(instr_hash);
  if (itr != loss_model_.end()) {
    return itr->second->OnExecution(priceup, pricedown, placedsize, direction,
                                    duration);
  }
  return false;
}
...
/**
 * @brief Each subscribed instrument has its own model instance
 */
