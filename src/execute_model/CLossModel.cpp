...
bool CLossModel::init(string instr, string config) {
  ...
  if (CTradeBaseInfo::instr_info_.find(instr_hash_) ==
  ...
  ASSERT_RET(loadLossModelConfig(
                 param_reader["/ExecModel/loss_model_conf_path"_json_pointer]),
             false);
  ASSERT_RET(
      dm_.init(param_reader["/ExecModel/drift_model_conf_path"_json_pointer]),
      false);
  ASSERT_RET(
      lq_.init(
          param_reader["/ExecModel/liquidity_model_conf_path"_json_pointer]),
      false);
  ASSERT_RET(
      lp_.init(param_reader["/ExecModel/logitp_model_conf_path"_json_pointer]),
      false);
  ASSERT_RET(ms_.init(instr.c_str()), false);
  ASSERT_RET(
      vm_.init(
          param_reader["/ExecModel/volatility_model_conf_path"_json_pointer]),
      false);
  ASSERT_RET(
      bs_.init(
          param_reader["/ExecModel/basesignal_model_conf_path"_json_pointer]),
      false);
...
bool CLossModel::loadLossModelConfig(string config) {
  ...
  string content((std::istreambuf_iterator<char>(f1)),
...
int CLossModel::GetSizeplacedOnPrice(
    tExecOrder &eo, int orderprice) {  // get the waiting size ahead
  int placedsize = 0;
...
double CLossModel::Lossfun(tExecOrder &eo, int level_index,
                           int orderprice) {  //
  double adapted_mut =
      LinearInterpolate(dm_.RetStartHorizon(), dm_.RetStartMut(),
                        dm_.RetEndHorizon(), dm_.RetEndMut(), eo.timeleft_);
  ...
  double factor0 = extra_slippage_ +
                   eo.direction_ * abm_.ExpectedSt(adapted_mut, sigma_t, delta,
                                                   spread / 2.0, eo.direction_);
  int placedsize = GetSizeplacedOnPrice(eo, orderprice);
  double factor2 =
      lp_.EvaluateProb(level_index, placedsize, bs_, eo.timeleft_,
                       valid_duration_, eo.direction_);  // prob multi-size
  double ExeProb = eo.direction_ > 0
                       ? abm_.GetBuyCABMExecutedProb(adapted_mut, sigma_t,
                                                     delta + spread / 2.0)
                       : abm_.GetSellCABMExecutedProb(adapted_mut, sigma_t,
                                                      delta + spread / 2.0);
  ...
  LOG_LVL(DEBUGLEVEL3,
          "current_mu: %f, current sigma: %f, Estimated_St: %f, "
          "neutral_Estimated_Prob: %f, ABM_Estimated_Prob: %f",
          adapted_mut, sigma_t, factor0, factor2, ExeProb);
  LOG_LVL(DEBUGLEVEL3,
          "Estimated Executed Loss: %f, Estimated Non-Executed Loss: %f",
          factor1 * factor2, factor0 * (1.0 - factor2));

  return factor1 * factor2 + factor0 * (1.0 - factor2);
}

int CLossModel::GetLossMinimizer(
    tExecOrder &eo, int newplacedsize) {  // argmin optimizer over price
  ...
  for (int level_index = 0; level_index < TOTAL_DELTA_; level_index++) {
    ...
    int midpriceT2 = ms_.askp_[0] + ms_.bidp_[0];
    int orderprice =
        GetExecutablePrice(midpriceT2, levels_[level_index], eo.direction_);
  ...
  for (int level_index = 0; level_index < TOTAL_DELTA_; level_index++) {
    LOG_LVL(DEBUGLEVEL3, "Estimated Loss for %d : %f", levels_[level_index],
            eo.loss_level_[level_index]);
  }
...
void CLossModel::GetBestOrderPriceVolume(tExecOrder &eo) {  //
  if (eo.timeleft_ > DURATION_LIMIT) {
    ...
    if (bestdelta == -2 and
        ((eo.loss_level_[0] - eo.loss_level_[bestlevelIndex]) <
         LOSSVALUE_DIFF_THRESHOLD) and
        (ms_.askp_[0] - ms_.bidp_[0] == 1) and (ms_.askv_[0] < ms_.bidv_[0])) {
      bestdelta = 1;
      bestlevelIndex = 0;
    }

    int bestorderprice =
        GetExecutablePrice(midpriceT2, bestdelta, eo.direction_);
    if (eo.direction_ > 0 and (ms_.bidp_[0] == eo.priceup_) and
        (bestlevelIndex == 0) and (ms_.askp_[0] - ms_.bidp_[0] == 1)) {
      bestlevelIndex = -1;
    } else if (eo.direction_ < 0 and (ms_.askp_[0] == eo.pricedown_) and
               (bestlevelIndex == 0) and (ms_.askp_[0] - ms_.bidp_[0] == 1)) {
      bestlevelIndex = -1;
    }

    int ordervolume = lq_.GetOrdervolume(leastordervolume_, maxordervolume_,
                                         volume_multiplier_, bestlevelIndex,
                                         eo.direction_, dm_, vm_);
    ordervolume = bestorderprice > eo.priceup_
                      ? 0
                      : (bestorderprice < eo.pricedown_ ? 0 : ordervolume);
    bestorderprice =
        bestorderprice > eo.priceup_
            ? eo.priceup_
            : (bestorderprice < eo.pricedown_ ? eo.pricedown_ : bestorderprice);
...
void CLossModel::PlaceOrder(
    tExecOrder &eo) {  // after bestorderprice,bestordervolume, start to order
  emOffset em_status;
  ...
  if (eo.QorderID_size_ == 0) {  // if there is no pre-ordering, then order
    if (eo.bestordervol_ > 0) {
      ...
      long orderid =
          sendOrder(eo, eo.bestorderprice_ * ticksize_, eo.bestordervol_,
                    direction_char, (int)em_status);
    ...
    if (APIStatus == 1) {  // if successfully sent to exchange to cancel
      if ((eo.bestordervol_ > eo.placed_vol_) and
          (eo.QorderID_size_ < MAX_OUTSTANDING_ORDERS)) {
        char direction_char = eo.direction_ > 0 ? AT_CHAR_Buy : AT_CHAR_Sell;
        long orderid = sendOrder(eo, eo.bestorderprice_ * ticksize_,
                                 eo.bestordervol_ - eo.placed_vol_,
                                 direction_char, (int)em_status);
        int non_occupied_count = 0;
        while (eo.QorderID_[non_occupied_count] > 0) ++non_occupied_count;
...
int CLossModel::CancelOrderAPIStatus(
    tExecOrder &eo, double to_orderprice) {  // check if it needs to cancel
  int api_status = 1;
  int count = 0;
  ...
  for (auto &eo : orders_) {
    ...
    switch (rtn->msg_type) {
      case (int)emOrderRtnType::EXECUTION: {
        ...
        LOG_LVL(DEBUGLEVEL3, "execution, execute price: %lf, traded: %d ",
                rtn->price, rtn->vol);
        break;
      }
      case (int)emOrderRtnType::CANCELED: {
        ...
        if (eo.cancel_status_ == CANCEL_STATUS_NORMAL) {
          ...
          eo.QorderID_[count] = -1;
          --eo.QorderID_size_;
          if (eo.QorderID_size_ == 0) {  // if all orders are cancelled, then
                                         // reorder, orders after cancel orders;
            if (eo.bestordervol_ > 0 && eo.totalsize_ > 0) {
              char direction_char =
                  eo.direction_ > 0 ? AT_CHAR_Buy : AT_CHAR_Sell;
              long orderid =
                  sendOrder(eo, eo.bestorderprice_ * ticksize_,
                            (eo.bestordervol_ < eo.totalsize_ ? eo.bestordervol_
                                                              : eo.totalsize_),
                            direction_char, (int)emOffset::AUTO);
              LOG_LVL(DEBUGLEVEL3,
                      "cancel reorder: %f ,vol: %d, size left: %d ",
                      eo.bestorderprice_ * ticksize_,
                      (eo.bestordervol_ < eo.totalsize_ ? eo.bestordervol_
                                                        : eo.totalsize_),
                      eo.totalsize_);
              eo.QorderID_[0] = orderid;
...
void CLossModel::GetTimeleft() {
  ...
  if (lst_tick_nano_ > 0 &&
      cur_md_nano >
          lst_tick_nano_ + 10 * 60 * 1000000000L)  // across time break > 10min
  {
  ...
  for (auto &eo : orders_) {
    if (eo.isOrdering_) {
      eo.timeleft_ = eo.duration_ -
                     (cur_md_nano / 1000000000.0 - eo.order_placetime_) +
                     extra_time;
      if (eo.timeleft_ >= valid_duration_) {
        eo.timeleft_ = valid_duration_;
      }
      LOG_LVL(DEBUGLEVEL0, "ExecutionOrder %d: timeleft: %lf", eo.order_id_,
              eo.timeleft_);
    }
  }
}
...
void CLossModel::sys_on_time(long nano) { ProcessExit(); }

void CLossModel::sys_on_switch_day(string day) {}

int CLossModel::OnExecution(
    double priceup, double pricedown, int placedsize, int direction,
    double duration) {  // for higher level strategy usage
  ...
  ot.order_id_ = ret_id;
  LOG_LVL(DEBUGLEVEL1, "%s|OnExecution %d: priceup:%lf, pricedown:%lf",
          parseNano(cur_md_nano, "%Y%m%d-%H:%M:%S").c_str(), ret_id, priceup,
          pricedown);
  GetBestOrderPriceVolume(ot);
  PlaceOrder(ot);
  return ret_id;
