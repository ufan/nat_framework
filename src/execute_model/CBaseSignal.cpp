...
bool CBaseSignal::init(string config_path)
{
    ...
    for(int count =0; count < HORIZON_USE_; count++){
        horizons_[count] = param_reader["horizon"][count];        
    }
    OBD_alpha_ = 0.1;
    return true;
}
...
void CBaseSignal::OnTick(const CMarketStruct & ms){
    ...
    for (int count=0; count < TOTAL_HORIZON_; count++){
        LOG_LVL(DEBUGLEVEL1, "trade%d: %f ", (int)horizons_[count], trade_[count]);
        //LOG_LVL(DEBUGLEVEL, "OBD%d : %f ", horizons_[count], OBD_[count]);
        LOG_LVL(DEBUGLEVEL1, "book%d: %f ", (int)horizons_[count], book_[count]);
        LOG_LVL(DEBUGLEVEL1, "CR%d: %f ", (int)horizons_[count], CR_[count]);
    }
    
    LOG_LVL(DEBUGLEVEL1, "tradeaskvol: %f, tradebidvol: %f", realtradeaskvol_, realtradebidvol_);
    LOG_LVL(DEBUGLEVEL1, "bookbid: %f, bookask: %f", bookbid_, bookask_);
    LOG_LVL(DEBUGLEVEL1, "askvol: %f, bidvol: %f, logskew: %f", composite_askvol_, composite_bidvol_, composite_logskew_);
    LOG_LVL(DEBUGLEVEL1, "avgTradeBidVol: %f, avgTradeAskVol: %f", avgTradeBidVol_, avgTradeAskVol_);
...
} // subject to the further revision for multi-level */
