...
const uint8_t g_xorkey[] = {0x20, 0x17, 0x10, 0x09, 0xff, 0xff, 0xff, 0xff, 0x13, 0x48, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88};
...
public:
	...
	/*
	   source length cannot exceeds RSA_size(rsa) - 11, see the openssl document for detail.
	   for 1024 bit key, maximum length is 128 - 11 = 117
	*/
	string rsaPublicEncrypt(string &source);
	string rsaPublicDecrypt(string &encrypted);

	string rsaPrivateEncrypt(string &source);
	string rsaPrivateDecrypt(string &encrypted);

	void gen3DesKey();
	const char* get3DesKeyBlock() {return (const char *)deskey_;}
	void set3DesKey(const char *p);
