...
/**
 * @brief Recover the true key bytes.
 * Original bytes are XOR encrypted, thus using the same bytes and XOR to
 * recover.
 */
...
string CCrypt::rsaPublicEncrypt(string &source) {
  ...
  if (rsa == NULL) {
    ...
    int iret =
        RSA_public_encrypt((int)source.size(), (unsigned char *)source.c_str(),
...
string CCrypt::rsaPrivateDecrypt(string &encrypted) {
  ...
  if (rsa == NULL) {
    ...
    int iret = RSA_private_decrypt(
        (int)encrypted.size(), (unsigned char *)encrypted.c_str(),
...
string CCrypt::rsaPrivateEncrypt(string &source) {
  ...
  if (rsa == NULL) {
    ...
    int iret =
        RSA_private_encrypt((int)source.size(), (unsigned char *)source.c_str(),
                            (unsigned char *)p_en, rsa, RSA_PKCS1_PADDING);
    if (iret < 0) {
      LOG_ERR("encrypt err");
    } else {
...
string CCrypt::desEncrypt(string &source) {
  ...
  for (uint32_t i = (source.size() + 1) / 8; i; --i) {
    DES_ecb3_encrypt((const_DES_cblock *)from, (DES_cblock *)to, &ks_[0],
                     &ks_[1], &ks_[2], DES_ENCRYPT);
    from += 8;
    to += 8;
  }
  ...
  if (rest_len) {
    ...
    DES_ecb3_encrypt((const_DES_cblock *)&tmp, (DES_cblock *)to, &ks_[0],
                     &ks_[1], &ks_[2], DES_ENCRYPT);
  }
