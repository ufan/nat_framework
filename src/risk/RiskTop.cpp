...
void RiskTop::onNew(int dir, int off, double px, int vol, uint32_t instr_hash, long order_ref)
{
	acc_base_.onNew(dir, off, px, vol, instr_hash);
	map_risk_instr_[instr_hash].onNew(dir, px, order_ref, CTimer::instance().getNano());
//	LOG_DBG("[%ld]NEW, %s, %s, %.2lf, %d", order_ref, getDirString(dir), getOffString(off), px, vol);
}
...
//		LOG_DBG("[%ld]CANCEL, %s, %s, %.2lf, %d", p_ord_trk->order_ref, getDirString(p_rtn_msg->dir), getOffString(p_rtn_msg->off), p_ord_trk->price, p_rtn_msg->vol);
	}
	if ( p_rtn_msg->msg_type & ODS(EXECUTION) )
	{
		map_risk_instr_[p_ord_trk->instr_hash].onTrd(p_ord_trk);
		LOG_DBG("[%ld]EXECUTION, %s, %s, %.2lf, %d", p_ord_trk->order_ref, getDirString(p_rtn_msg->dir), getOffString(p_rtn_msg->off), p_rtn_msg->price, p_rtn_msg->vol);
	}
}
