...
int getDailyCycle()
{
	return AT_INT_DAILY_CYCLE;
}

int getSessionCycle()
{
	return AT_INT_SESSION_CYCLE;
}
...
void BarMaker::generateVecBar(tInstrumentInfo* p_instr_info, int cycle_sec)
{
	...
	if (cycle_sec == AT_INT_DAILY_CYCLE)
	{
	...
	else if (cycle_sec == AT_INT_SESSION_CYCLE)
	{
		for (auto it = vec_ctts.begin(); it < vec_ctts.end(); ++it)
		{
			bar.bob = it->bgn_sec;
...
//	{
//		printf("vec_bar[%d/%lu] %d %02ld:%02ld:%02ld(%02ld:%02ld:%02ld) %02ld:%02ld:%02ld(%02ld:%02ld:%02ld), settle: %02ld:%02ld:%02ld\n"
//			, cnt, vec_bar.size(), it->is_auction 
//			, (18+it->bob/3600)%24, it->bob%3600/60, it->bob%60
//			, (18+it->adjust_bob/3600)%24, it->adjust_bob%3600/60, it->adjust_bob%60
...
void BarMaker::OnTick(const UnitedMarketData* p_umd)
{
	...
	while (true)
	{
		if (pos == 0)
		{
			...
			{
//				printf("in_cur_bar, need_update_pre, %s\n", p_bar->instr_str);
...
vector<TimeSpan> BarMaker::getTradingTimespan(string prd)
{
	...
	else if (prd == "AP" || prd == "JR" || prd == "LR" || prd == "PM" || prd == "RI"
			|| prd == "RS" || prd == "SF" || prd == "SM" || prd == "WH"
			|| prd == "bb" || prd == "c"  || prd == "cs" || prd == "fb" || prd == "jd"
			|| prd == "l"  || prd == "pp" || prd == "v"
			|| prd == "fu" || prd == "wr")
	{
	...
	else if (prd == "CF" || prd == "CY" || prd == "FG" || prd == "MA" || prd == "OI"
			|| prd == "RM" || prd == "SR" || prd == "TA" || prd == "ZC"
			|| prd == "a" || prd == "b" || prd == "i" || prd == "j" || prd == "jm"
			|| prd == "m" || prd == "p" || prd == "y")
	{
	...
	else if (prd == "al" || prd == "cu" || prd == "ni" || prd == "pb" || prd == "sn" || prd == "zn")
	{
		ts.bgn_sec = BarHelper::getBaseIntervalFromBjtSec(20 * 3600 + 59 * 60);
		ts.end_sec = BarHelper::getBaseIntervalFromBjtSec(21 * 3600 + 0 * 60);
		ts.is_auction = true;
...
Bar* BarMaker::GetBar()
{
//	printf("%d, %d, %d, %ld\n", cycle_sec_, rtn_bar_idx, bar_idx, last_umd.exch_time);
//
//	if (bar_idx != -1 && bar_idx < vec_bar.size())
//	{
//		Bar2* p_bar = &vec_bar[bar_idx];
//		long exch_sec = BarHelper::getBaseIntervalFromNano(*p_exch_nano);
//		if (exch_sec - p_bar->eob > DELAY_SEC && !p_bar->is_auction)
//		{
//			if (last_umd.instr_hash != 0)
//			{
//				p_bar->settle(&last_umd);
