...
CSignalRequester::CSignalRequester(long timeout, uint32_t buf_size) : CService(buf_size, 4096), timeout_(timeout)
{
...
{
	is_connect_ = false;
	LOG_INFO("%s:%d connect err, fd: %d, err: %s", ip_.c_str(), port_, fd_, strerror(err));
	CService::close();
	if(err == ETIMEDOUT)
	{
		connect(ip_, port_, timeout_ / 1000000000L);
...
{
	if(is_connect_ && (lst_heartbeat_time_ + (timeout_ >> 1) < g_system_time_ns))
	{
		tSACmdHead head;
