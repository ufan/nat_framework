...
umap<uint32_t, CStrategyProcess::tSubscribedInstrInfo> CStrategyProcess::instr_info_map_;
...
bool CStrategyProcess::loadConfig(string config_content)
{
	...
	p_md_helper_ = CMDHelperFactory::create(md_helper_conf["type"], j_conf["name"]);
	...
	p_td_helper_ = CTDHelperFactory::create(td_helper_conf["type"], j_conf["name"]);
	...
	setSwitchDayCallBack(bind(&CStrategyProcess::pre_on_switch_day, placeholders::_1));
	return true;
}
...
int CStrategyProcess::add(CStrategyBase* stg, int profile_id)
{
	...
	if(profile_id >= acc_conf.size()) acc_conf = j_conf["AccountDefault"];
	else acc_conf = acc_conf[profile_id];
	stg->p_risk_stg_.reset(new RiskStg);
	string stg_name = j_conf["name"].get<string>() + string(SUB_ACCOUNT_FILE_SUFFIX) + to_string(profile_id);
...
bool CStrategyProcess::subsBar(CStrategyBase* stg, string instrument, long interval_min)
{
	if(!subscribe(stg, instrument)) return false;
	for(auto &i : subs_bar_info_)
	{
		if(i.first.getCycle() == interval_min && i.first.getInstr() == instrument)
		{
	...
	subs_bar_info_.emplace_back(BarMaker(&info, interval_min), vector<CStrategyBase*>{stg});
	return true;
}
